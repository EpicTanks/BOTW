...
public class PlayerParty extends DungeonObject {
 ...
 // Does something based on the input. Returns false if the turn is not
 // passed.
 public boolean takeAction(KeyEvent e) {
  switch (e.getKeyCode()) {
  case 32:
   BestOfTheWest.getLevel().useStairs(x, y); // use stairs with .
  ...
  case 67:
   if (CharacterSheet.selectedAbility != null) {
    if (CharacterSheet.selectedAbility.getMPCost() > CharacterSheet.caster.getMP()) {
 ...
 private boolean changeCasting() {
  ...
  if (CharacterSheet.selectedAbility.getGood() == true && !isCasting) {
   BestOfTheWest.c.addMessage(CharacterSheet.caster.getName() + " casts a HEALING spell!");
 ...
 private void castInDirection(int xmod, int ymod) {
  ...
  BestOfTheWest.c.addCloseMessage(CharacterSheet.caster.getName() + " casts a spell...");
 ...
 public CharacterSheet getFirstAlive() {
  if (BestOfTheWest.sheets[0].isAlive()) {
   ...
   return BestOfTheWest.sheets[2];
     ...
     BestOfTheWest.m.changeTrack("GameOver"); 
