		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				if (contents[i][j] == null) {
					...
					// if there is something in the inventory, proceed to draw
					// it in the CORRECT PLACE
					contents[i][j].paint(g2d, x + (5 * ((j % 2) + 1)) + (j % 2) * 64,
							y + (5 * ((i % 2) + 1)) + (i % 2) * 64);
				}
			}
		}
	...
	public void click(int x, int y) {
		...
		if (cMenuVis == true) {
			if (x > (this.x + 210) && x < (this.x + 260) && y > (this.y + 50) && y < (this.y + 70)) {
				...
				// check to see if the selected item slots is the STUPID DUMMY
				// number
				if (selectedItemS1 == 3 || selectedItemS2 == 3) {
					cMenuVis = false;
				} else {
				...
				// ystem.out.println("IM GONNA MOVE");
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int j = 0; j < 2; j++) {
				...
				if (contents[i][j] == null) {
					// System.out.println("There is nothing in slot " +
					// ((i*2)+j));
				} else {
					// dev code
					// System.out.println("Item, Slot:" +
					// contents[i][j].getName() + "," +
					// contents[i][j].getSlot());
