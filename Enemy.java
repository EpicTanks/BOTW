...
public class Enemy extends DungeonObject {
	...
	// Take an amount of damage. Also calls the method to remove this enemy from
	// the level.
	...
	private int shortestPath(int moves, int cX, int cY) {
		...
		moves--;
		return max4(shortestPath(moves, cX + 1, cY), shortestPath(moves, cX - 1, cY), shortestPath(moves, cX, cY + 1),
				shortestPath(moves, cX, cY - 1));
	}
