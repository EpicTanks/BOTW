...
public class CharacterSheet {
	...
	public void loadImages(String n) {
		try {
			portrait = ImageIO.read(
					new File("images/portraits/colour" + ((int) (Math.random() * 2) + 1) + "/" + race + "_male.png"));
	...
	public void click(int x, int y) {
		...
		// System.out.println("top(y must b lesr): " + ( (partyOrder*75)+5) + ".
		// bottom(y must be bigr):" + ((partyOrder*75)+40));
		...
		if (statVis == true) {
			for (int k = 0; k < abilities.length; k++) {
				if (x >= 197 && x <= 320 && y >= 7 + ((partyOrder * 70) + (partyOrder * 5) + ((k * 22) + (k * 5)))
						&& y <= ((partyOrder * 70) + (partyOrder * 5) + ((k * 22) + (k * 5))) + 29
						&& abilities[k] != null) {
					if (selectedSheet == this && hp > 0) {
						BestOfTheWest.c.clear();
						BestOfTheWest.c
								.addMessage(name + " has prepared " + abilities[k].toString() + ", press 'C' to cast.");
	...
	public int rollDamage() {
		return ((int) (Math.random() * str) + 1) + weap.rollDamage(); // 1 to
																		// str
																		// damage
	}
